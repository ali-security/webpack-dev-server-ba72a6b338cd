...
class Server {
  ...
  async initialize() {
    ...
    const hmrPath = webSocketServerOptions && webSocketServerOptions.path;
    ...
    (this.webSocketProxies).forEach((webSocketProxy) => {
      ...
      (this.server).on("upgrade", (req, socket, head) => {
        // The dev server's own web socket must never be forwarded to a
        // user-defined proxy, otherwise the proxy target becomes the HMR server
        // for the page. Match the configured path exactly the same way the
        // underlying web socket server (`ws`) does in
        // `WebSocketServer#shouldHandle`: a raw, case-sensitive comparison of
        // the request target with the query string stripped. Any normalization
        // here would classify URL variants (`//ws`, `/WS`, ...) as the HMR
        // socket even though `ws` itself refuses them.
  ...
  setupMiddlewares() {
    ...
    middlewares.push({
      ...
      middleware: (req, res, next) => {
        ...
        if (this.checkHeader(headers, headerName, false, false)) {
          next();
          return;
        }
    ...
    if (this.options.proxy) {
      ...
      const getProxyMiddleware = (proxyConfig) => {
        // `http-proxy-middleware` subscribes to the server's "upgrade" event on
        // its own as soon as a request has passed through it, and that
        // subscription forwards every upgrade - including the dev server's own
        // web socket - to the proxy target. Web socket upgrades are dispatched
        // explicitly through `middleware.upgrade` (see `initialize`), which
        // keeps the dev server web socket out of the proxy, so the internal
        // subscription is turned off in favor of it.
        const proxyMiddlewareOptions = { ...proxyConfig, ws: false };
  ...
  checkHeader(headers, headerToCheck, allowIP, validateHost = true) {
    ...
    // a malformed value (i.e. an invalid IPv6 literal) makes the parser throw,
    // and an unhandled throw here would take down the whole dev server
    /** @type {string | null} */
    let hostname;
    ...
    // `validateHost` is `false` when only an explicitly configured
    // `allowedHosts` entry should let the request through, i.e. the implicit
    // localhost/listening-host allowances above must not apply.
    if (validateHost && isValidHostname) {
      return true;
    }
  ...
  isSameOrigin(headers) {
    ...
    // The `origin` header is set by the browser and can't be spoofed by a page,
    // so an `origin` whose hostname equals the hostname the request was sent to
    // means the page was served by the dev server itself.
